...
struct Net_config
{
	...
	char* modelbuffer_ptr;
	size_t modelbuffer_size;
	string model_name;
};
...
YOLO::YOLO(Net_config config)
{
	...
	this->net = readNetFromONNX(config.modelbuffer_ptr, config.modelbuffer_size);
	...
	if (endsWith(config.model_name, "6.onnx"))
	{
...
{
	/*
	ifstream modelStream("weights/yolov5m.onnx", ios::binary);
	...
	modelStream.read(modelData, bufferSize);
        */
	//extern char foo_data[] asm("_binary_weights_yolov5m_onnx_start") ;
	//extern char foo_data_size[]  asm("_binary_weights_yolov5m_onnx_size");
	//extern char foo_data_end[]  asm("_binary_weights_yolov5m_onnx_end");
	extern char _binary_weights_yolov5m_onnx_start[];
	extern char _binary_weights_yolov5m_onnx_size[];
	extern char _binary_weights_yolov5m_onnx_end[];
	size_t foo_size = (size_t)(_binary_weights_yolov5m_onnx_end) - (size_t)(_binary_weights_yolov5m_onnx_start);
	//char  *foo_copy = (char*)malloc(foo_size);
	//memcpy(foo_copy, foo_data, foo_size);
	//cout<<size_t(_binary_weights_yolov5m_onnx_start)<<endl<<foo_size<<endl<<size_t(_binary_weights_yolov5m_onnx_end)<<endl;
	string model_name = "yolov5m.onnx";
	Net_config yolo_nets = { 0.3, 0.5, 0.3,_binary_weights_yolov5m_onnx_start, foo_size ,model_name};
